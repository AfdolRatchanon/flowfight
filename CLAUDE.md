...
### Deploy หลัง Tests ผ่าน (อัตโนมัติ)
เมื่อ tests ผ่านทั้งหมดแล้ว ให้รัน deploy ขึ้น Firebase ด้วย:
```bash
npx tsc --noEmit          # ตรวจ TypeScript types
npm run build             # build production
firebase deploy           # deploy hosting + firestore rules พร้อมกัน
```

**หมายเหตุ:**
- ถ้าแก้เฉพาะ `firestore.rules` → `firebase deploy --only firestore:rules`
- ถ้าแก้เฉพาะ frontend → `firebase deploy --only hosting`
- ถ้าแก้ทั้งคู่ → `firebase deploy` (ไม่ต้องระบุ flag)
- ห้าม deploy ถ้า `npm test` หรือ `npx tsc --noEmit` มี error
...
```
1. ปวส. content (Phase 2)  — 10 ด่าน advanced (level_21–30)
2. Phase 3 Analytics       — Teacher Dashboard + Auto-grading ก่อน
3. Phase 4 Platform        — Mobile / PWA / Sandbox
4. Phase 5 Advanced LMS    — Custom Level Editor, Class Competition
5. ประถม / มัธยม           — ทำสุดท้าย (ต้องออกแบบ UX ใหม่ทั้งหมด)
```

**กฎ:** ทุกครั้งที่เริ่ม feature ใหม่ → อัปเดต ROADMAP.md เปลี่ยน `[ ]` เป็น `[x] vX.Y.Z`

## Current Version
**v0.20.0** — ดู ROADMAP.md สำหรับ Phase ถัดไป
